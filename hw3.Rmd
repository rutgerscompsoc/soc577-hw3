...
Q.11. Complete the `stm` function to run an initial topic modeling. Pick a value for `K` and add arguments to allow prevalence to vary as a function of month (either linear or non-linear) and party, and content to vary as a function of party.
...
Q. 14 Use the`estimateEffect` function covered in lecture to analyze the relationship between the covariate and the topic. 
...
Q.15 Select a topic of interest and plot the differences in content by partisanship. Answer the question below.
```{r q15, echo=TRUE, tidy=TRUE}
